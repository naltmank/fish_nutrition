...
# read in inferred species from Katrina's research
inferred <- read.csv(here::here("data", "inferred_species_list.csv"), na.strings=c("","NA"))

# extract species that were IDed to lowest taxonomic resolution, concatenate inferred species, remove duplicates
target_species <- unique(c(unique(na.omit(cnh$species)), inferred$species))
...
#             Chisq Df Pr(>Chisq)    
# Year       102.4204  1  < 2.2e-16 ***
# Coast        7.2012  2    0.02731 *  
# Year:Coast  33.0234  2  6.746e-08 ***
...
cc_fisheries_biomass <- cc_fisheries_biomass %>%
  mutate(Year_centered = ifelse(Cyclone == "Pre-disturbance", Year - 2006, Year - 2011))

bl_fisheries_biomass <- bl_fisheries_biomass %>%
  mutate(Year_centered = ifelse(Bleaching == "Pre-disturbance", Year - 2016, Year - 2020))
...
#                               Chisq Df Pr(>Chisq)    
# Year_centered                0.0630  1  0.8017850    
# Cyclone                     22.3122  1  2.317e-06 ***
# Coast                        9.2740  2  0.0096868 ** 
# Year_centered:Cyclone        6.8160  1  0.0090344 ** 
# Year_centered:Coast          6.7083  2  0.0349394 *  
# Cyclone:Coast               17.7883  2  0.0001372 ***
# Year_centered:Cyclone:Coast  0.4060  2  0.8162796    
performance::r2(cc_fisheries_mod) # R2m = 0.16, R2c = 0.54
plot(allEffects(cc_fisheries_mod))
...
#  East - North   0.1585 0.1055 Inf   1.503  0.2897
#  East - West    0.0743 0.1110 Inf   0.669  0.7812
#  North - West  -0.0842 0.1028 Inf  -0.819  0.6911
# 
# Cyclone = Pre-disturbance:
#  contrast     estimate     SE  df z.ratio p.value
#  East - North   0.2066 0.1018 Inf   2.028  0.1054
#  East - West    0.1696 0.0996 Inf   1.702  0.2043
#  North - West  -0.0370 0.1030 Inf  -0.359  0.9314

emtrends(cc_fisheries_mod, pairwise ~ Cyclone | Coast, var = "Year_centered")
# $contrasts
# Coast = East:
#  contrast                               estimate    SE  df z.ratio p.value
#  (Post-disturbance) - (Pre-disturbance)    0.109 0.106 Inf   1.025  0.3055
# 
# Coast = North:
#  contrast                               estimate    SE  df z.ratio p.value
#  (Post-disturbance) - (Pre-disturbance)    0.157 0.101 Inf   1.551  0.1208
# 
# Coast = West:
#  contrast                               estimate    SE  df z.ratio p.value
#  (Post-disturbance) - (Pre-disturbance)    0.204 0.105 Inf   1.940  0.0524

# extract info needed to plot slopes
cyclone_emm <- emmeans::emmip(cc_fisheries_mod,
                   Cyclone | Coast ~ Year_centered,
                   at = list(Year_centered = seq(1,5,0.01)), plotit = F, CIs = T)
...
cyclone_emm <- cyclone_emm %>%
  mutate(Year = ifelse(Cyclone == "Pre-disturbance", Year_centered + 2005, Year_centered + 2009.01))
...
#                                 Chisq Df Pr(>Chisq)   
# Year_centered                  1.4993  1   0.220786   
# Bleaching                      9.8649  1   0.001685 **
# Coast                          4.4570  2   0.107690   
# Year_centered:Bleaching        0.1600  1   0.689151   
# Year_centered:Coast            1.8432  2   0.397877   
# Bleaching:Coast               13.3504  2   0.001262 **
# Year_centered:Bleaching:Coast  2.9230  2   0.231883   
...
#  contrast     estimate    SE  df z.ratio p.value
#  East - North  -0.2505 0.125 Inf  -2.004  0.1112
#  East - West   -0.2169 0.124 Inf  -1.755  0.1850
#  North - West   0.0336 0.121 Inf   0.277  0.9586
# 
# Bleaching = Pre-disturbance:
#  contrast     estimate    SE  df z.ratio p.value
#  East - North   0.0220 0.122 Inf   0.181  0.9821
#  East - West    0.0341 0.128 Inf   0.266  0.9617
#  North - West   0.0121 0.125 Inf   0.097  0.9948

emtrends(bl_fisheries_mod, pairwise ~ Bleaching | Coast, var = "Year_centered")
# $contrasts
# Coast = East:
#  contrast                               estimate    SE  df z.ratio p.value
#  (Post-disturbance) - (Pre-disturbance)  -0.2071 0.127 Inf  -1.633  0.1025
# 
# Coast = North:
#  contrast                               estimate    SE  df z.ratio p.value
#  (Post-disturbance) - (Pre-disturbance)   0.0654 0.120 Inf   0.543  0.5868
# 
# Coast = West:
#  contrast                               estimate    SE  df z.ratio p.value
#  (Post-disturbance) - (Pre-disturbance)   0.0439 0.126 Inf   0.349  0.7272

# extract info needed to plot slopes
bleaching_emm <- emmeans::emmip(bl_fisheries_mod,
                   Bleaching | Coast ~ Year_centered,
                   at = list(Year_centered = seq(1,4,0.01)), plotit = F, CIs = T)
# back-transform modeled response variable 
bleaching_emm$yvar_trans <- exp(bleaching_emm$yvar)
# backtransform year_centered to the actual year for plotting
bleaching_emm <- bleaching_emm %>%
  mutate(Year = ifelse(Bleaching == "Pre-disturbance", Year_centered + 2015, Year_centered + 2018.01))
...
(cyclone_biomass <-
   ggplot() +
   geom_line(data = subset(cyclone_emm, Cyclone == "Pre-disturbance"), aes(x = Year, y = yvar_trans, colour = Coast)) +
   geom_ribbon(data = subset(cyclone_emm, Cyclone == "Pre-disturbance"),
               aes(x = Year, ymin = exp(LCL), ymax = exp(UCL), fill = Coast), alpha = 0.5) +
   geom_line(data = subset(cyclone_emm, Cyclone == "Post-disturbance"), aes(x = Year, y = yvar_trans, colour = Coast)) +
   geom_ribbon(data = subset(cyclone_emm, Cyclone == "Post-disturbance"),
               aes(x = Year, ymin = exp(LCL), ymax = exp(UCL), fill = Coast), alpha = 0.5) +
   geom_vline(xintercept = 2010, linetype = 2, colour = "black", size = 1) +
   labs(y = "Estimated Biomass (g) \n",
...
(bleaching_biomass <-
   ggplot() +
   geom_line(data = subset(bleaching_emm, Bleaching == "Pre-disturbance"), aes(x = Year, y = yvar_trans, colour = Coast)) +
   geom_ribbon(data = subset(bleaching_emm, Bleaching == "Pre-disturbance"),
               aes(x = Year, ymin = exp(LCL), ymax = exp(UCL), fill = Coast), alpha = 0.5) +
  geom_line(data = subset(bleaching_emm, Bleaching == "Post-disturbance"), aes(x = Year, y = yvar_trans, colour = Coast)) +
   geom_ribbon(data = subset(bleaching_emm, Bleaching == "Post-disturbance"),
               aes(x = Year, ymin = exp(LCL), ymax = exp(UCL), fill = Coast), alpha = 0.5) +
   geom_vline(xintercept = 2019, linetype = 2, colour = "red", size = 1) +
   labs(y = "Estimated Biomass (g) \n",
...
# ggsave(filename = "output/biomass_panel_v4.png", model_panel, height = 10, width = 13)
```
...
#Year         1    4.567 0.02846 26.2969  0.001 ***
#Coast        2    5.633 0.03510 16.2170  0.001 ***
#Year:Coast   2    1.597 0.00995  4.5987  0.001 ***
#Residual   856  148.665 0.92648                   
#Total      861  160.462 1.00000          
...
#                 Df SumOfSqs      R2       F Pr(>F)    
# Coast            2    4.641 0.05110 13.3575  0.001 ***
# Pre.post         1    3.382 0.03724 19.4659  0.001 ***
# Coast:Pre.post   2    0.800 0.00881  2.3018  0.002 ** 
# Residual       472   82.002 0.90286                   
# Total          477   90.825 1.00000                   
...
#                 Df SumOfSqs      R2      F Pr(>F)    
# Coast            2    2.376 0.03514 7.0265  0.001 ***
# Pre.post         1    0.924 0.01367 5.4656  0.001 ***
# Coast:Pre.post   2    0.403 0.00596 1.1918  0.289    
# Residual       378   63.897 0.94523                  
# Total          383   67.599 1.00000      
...
fish_ord_4 # stress = 0.15 - still high but much better - going 4D
...
(fish_nmds <- 
  ...
  geom_text(aes(x = 2, y = 1.1, label = "Stress = 0.15\nYear - P = 0.001\nCoast - P = 0.001\nYear X Coast - P = 0.001"),
            hjust = 1, size = 5) +
...
cc_fish_ord_4 # stress = 0.15 - best
...
bl_fish_ord_4 # stress = 0.14 - best
...
(cc_fish_nmds <-
  ...
  geom_path(cc_fish.cent, mapping = aes(x = NMDS1, y = NMDS2, colour = Coast),
            arrow = arrow(length=unit(0.4,"cm"), ends="last", type = "closed"), size = 1) +
  geom_text(aes(x = -1.8, y = -0.9, label = "Stress = 0.15\nDisturbance - P = 0.001\nCoast - P = 0.001\nDisturbance X Coast - P = 0.002"),
            hjust = 0, size = 4) +
...
(bl_fish_nmds <-
  ...
  geom_path(bl_fish.cent, mapping = aes(x = NMDS1, y = NMDS2, colour = Coast),
            arrow = arrow(length=unit(0.4,"cm"), ends="last", type = "closed"), size = 1) +
  geom_text(aes(x = 3.2, y = 1, label = "Stress = 0.14\nDisturbance - P = 0.001\nCoast - P = 0.002\nDisturbance X Coast - P = 0.29"),
            hjust = 1, size = 4) +
...
# ggsave(filename = "output/biomass_ordination_v2.png", biomass_ord_panel, height = 12, width = 11)
```
...
(omega_biomass <-
  ...
  geom_point(data = nutrients_and_biomass, aes(x = Biomass_total, y = Omega3_total, colour = Coast), size = 3) +
    geom_text(aes(x = 115000, y = 6, label = "tau == 0.96"), size = 8, parse = T, hjust = 1) +
   labs(y = "Omega 3 (mg) \n",
        x = "Biomass (g)", title = "b. Omega 3") +
  theme_classic()  +
   theme(plot.title = element_text(size = 22),
        panel.background = element_blank(), 
        panel.grid.major = element_blank(),  #remove major-grid labels
...
(calcium_biomass <-
  ...
  geom_point(data = nutrients_and_biomass, aes(x = Biomass_total, y = Calcium_total, colour = Coast), size = 3) +
    geom_text(aes(x = 115000, y = 3000, label = "tau == 0.90"), size = 8, parse = T, hjust = 1) +
   labs(y = "Calcium (mg) \n",
        x = "Biomass (g)", title = "c. Calcium") +
  theme_classic() +
   theme(plot.title = element_text(size = 22),
        panel.background = element_blank(), 
        panel.grid.major = element_blank(),  #remove major-grid labels
...
(vitaminA_biomass <-
  ...
  geom_point(data = nutrients_and_biomass, aes(x = Biomass_total, y = VitaminA_total, colour = Coast), size = 3) +
    geom_text(aes(x = 115000, y = 2000, label = "tau == 0.83"), size = 8, parse = T, hjust = 1) +
   labs(y = "Vitamin A (mg) \n",
        x = "Biomass (g)", title = "e. Vitamin A") +
  theme_classic() +
   theme(plot.title = element_text(size = 22),
        panel.background = element_blank(), 
        panel.grid.major = element_blank(),  #remove major-grid labels
...
(selenium_biomass <-
  ...
  geom_point(data = nutrients_and_biomass, aes(x = Biomass_total, y = Selenium_total, colour = Coast), size = 3) +
    geom_text(aes(x = 115000, y = 1000, label = "tau == 0.88"), size = 8, parse = T, hjust = 1) +
   labs(y = "Selenium (mg) \n",
        x = "Biomass (g)", title = "f. Selenium") +
  theme_classic() +
   theme(plot.title = element_text(size = 22),
        panel.background = element_blank(), 
        panel.grid.major = element_blank(),  #remove major-grid labels
...
(zinc_biomass <-
  ...
  geom_text(aes(x = 115000, y = 100, label = "tau == 0.75"), size = 8, parse = T, hjust = 1) +
   labs(y = "Protein (mg) \n",
        x = "Biomass (g)", title = "g. Zinc") +
  theme_classic() +
   theme(plot.title = element_text(size = 22),
        panel.background = element_blank(), 
        panel.grid.major = element_blank(),  #remove major-grid labels
...
# ggsave(filename = "output/biomass_nutrients_corr_v2.png", nutrient_corr_panel, height = 8, width = 20)
```
...
#             Df SumOfSqs      R2       F Pr(>F)    
# Coast        2   0.4751 0.03721 17.8388  0.001 ***
# Year         1   0.6346 0.04970 47.6528  0.001 ***
# Coast:Year   2   0.2600 0.02036  9.7612  0.001 ***
# Residual   856  11.3994 0.89273                   
# Total      861  12.7690 1.00000   
...
#                 Df SumOfSqs      R2       F Pr(>F)    
# Coast            2   0.5196 0.06531 17.6573  0.001 ***
# Pre.post         1   0.4345 0.05461 29.5284  0.001 ***
# Coast:Pre.post   2   0.0566 0.00711  1.9226  0.111    
# Residual       472   6.9446 0.87296                   
# Total          477   7.9552 1.00000    
...
#                 Df SumOfSqs      R2       F Pr(>F)    
# Coast            2   0.2274 0.05027 10.2711  0.001 ***
# Pre.post         1   0.0689 0.01524  6.2260  0.014 *  
# Coast:Pre.post   2   0.0431 0.00952  1.9443  0.126    
# Residual       378   4.1849 0.92498                   
# Total          383   4.5243 1.00000   
...
(nutrients_nmds <- 
  ...
  geom_path(nutrients.cent, mapping = aes(x = NMDS1, y = NMDS2, colour = Coast),
            arrow = arrow(length=unit(0.4,"cm"), ends="last", type = "closed"), size = 1) +
  geom_text(aes(x = -0.5, y = -0.115, label = "Stress = 0.04\nYear - P = 0.001\nCoast - P = 0.001\nYear X Coast - P = 0.001"),
            hjust = 0, size = 5) +
...
(cc_nutrients_nmds <-
  ...
  geom_path(cc_nutrients.cent, mapping = aes(x = NMDS1, y = NMDS2, colour = Coast),
            arrow = arrow(length=unit(0.4,"cm"), ends="last", type = "closed"), size = 1) +
  geom_text(aes(x = -0.5, y = -0.135,
                label = "Stress = 0.04\nDisturbance - P = 0.001\nCoast - P = 0.001\nDisturbance X Coast - P = 0.11"),
            hjust = 0, size = 4) +
  labs(title = "b. COTS and Cyclone") +
  scale_linetype_manual(values = c("Pre-disturbance" = "solid", "Post-disturbance" = "dashed")) + 
  theme_bw() + 
...
# ggsave(filename = "output/nutrients_ordination_v2.png", nutrients_ord_panel, height = 12, width = 11)
```
